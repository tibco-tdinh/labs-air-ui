...
export class CommonIotDataPipelineComponent implements OnInit, AfterViewInit {
  ...
  updateProtocolViewForm(protocolType, protocol) {
      ...
      if (protocolType == 'MQTT') {
          ...
          const delimInd = protocol.brokerURL.lastIndexOf(':');

          const hostname = protocol.brokerURL.substring(6, delimInd);
          const port = protocol.brokerURL.substring(delimInd + 1);
          ...
          const delimInd = protocol.brokerURL.lastIndexOf(':');
          const hostname = protocol.brokerURL.substring(0, delimInd);
          const port = protocol.brokerURL.substring(delimInd + 1);
  ...
  updateFilterViewForm(filter) {
      ...
      const selDevices = filter.deviceNames.split(',');
      let status = false;
  ...
  saveDataPipeline(deployPipeline: boolean) {
      ...
      const applicationId = 'iot-data-' + tsms;
      let pipelineStatus = 'Undeployed';

      // Check if deployment is needed
      if (deployPipeline) {

          const protocolObj = this.buildProtocolProperties(protocol, this.transportForm);
          const dataStoreObj = this.buildDataStoreProperties(dataStore, this.dataStoreForm);
          const filterObj = this.buildDataFilteringProperties(this.filteringForm);
          const streamingObj = this.buildStreamingProperties(this.streamingForm);
      ...
      const protocolUid = this.transportForm.get('uid').value;
      const dataStoreUid = this.dataStoreForm.get('uid').value;
      const graphFilterObj = this.buildGraphDataFilteringProperties(this.filteringForm);
      const graphStreamingObj = this.buildGraphStreamingProperties(this.streamingForm);
  ...
  undeploySelectedDataPipeline() {
      ...
      if (this.pipelineSelection.hasValue()) {
          ...
          // Undeploy pipeline
          this.flogoDeployService.undeploy(request as any)
              .subscribe(res => {
  ...
  deploySelectedDataPipeline() {
      ...
      if (this.pipelineSelection.hasValue()) {
          ...
          const protocolObj = this.buildProtocolProperties(pipeline.protocolType, this.transportViewForm);
          const dataStoreObj = this.buildDataStoreProperties(pipeline.dataStoreType, this.dataStoreViewForm);
          const filterObj = this.buildDataFilteringProperties(this.filteringViewForm);
          const streamingObj = this.buildStreamingProperties(this.streamingForm);
          // let loggingObj = this.buildLoggingProperties();

          const applicationId = pipeline.name;
          const env = [];
          env.push({ 'name': 'FLOGO_APP_PROPS_ENV', 'value': 'auto' });
          protocolObj.forEach(function (protocol) {
              env.push(protocol);
  ...
  buildDataFilteringProperties(form: FormGroup): any {
      ...
      devicesArray.controls.forEach((item: AbstractControl) => {
